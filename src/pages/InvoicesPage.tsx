...
const InvoicesPage: React.FC = () => {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const response = await axios.get<{ data: Invoice[] }>('https://findmybills-backend.vercel.app/api/invoices', {
        headers: {
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post('https://findmybills-backend.vercel.app/api/invoices', formData, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  const handleDownload = async (invoiceId: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get<Blob>(`https://findmybills-backend.vercel.app/api/invoices/${invoiceId}/download`, {
        headers: {
          Authorization: `Bearer ${token}`
        },
        responseType: 'blob'
