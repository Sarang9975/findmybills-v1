...
const InvoicesPage: React.FC = () => {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      const response = await axios.get<{ data: Invoice[] }>('http://localhost:5000/api/invoices', {
        headers: {
  ...
  const handleDownload = async (invoiceId: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get<Blob>(`http://localhost:5000/api/invoices/${invoiceId}/download`, {
        headers: {
          Authorization: `Bearer ${token}`
        },
        responseType: 'blob'
